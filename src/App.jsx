...
function PersonalDetails({go,plan}){if(!plan)return null;return <><Header title="Seu plano personalizado" onBack={()=>go('personalPlan')}/><main className="page"><section className="personalHero"><span>PLANO GERADO</span><h2>{plan.name}</h2><p>{plan.summary}</p><div className="planStats"><b>{plan.days} dias</b><b>{plan.level}</b><b>{plan.goal}</b></div></section><div className="weeklyPlan">{plan.week.map(day=><article className="dayCard" key={day.day}><div className="dayHead"><div><span>Dia {day.day}</span><h3>{day.title}</h3></div><small>{day.duration}</small></div>{day.exercises.map((e,i)=><div className="planExercise" key={e.id+'-'+i}><div><b>{e.name}</b><small>{e.muscle} • {e.sets} séries × {e.reps}</small></div><strong>{e.rest}s</strong></div>)}</article>)}</div><p className="muted planDisclaimer">Plano automático para adultos saudáveis. O IMC é apenas uma variável de triagem; ele não determina sozinho a escolha de exercícios. Em caso de dor, condição médica, lesão ou dúvida sobre segurança, procure um profissional qualificado.</p></main></>}

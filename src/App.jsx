...
export default function App(){const [screen,setScreen]=useState('home');const [selected,setSelected]=useState(programs[0]);const {history,profile,setProfile,saveWorkout,customWorkouts,saveCustomWorkout,cloud}=useWorkoutStorage();const go=(next,data)=>{if(data)setSelected(data);setScreen(next)};if(screen==='home')return <Home go={go} history={history} profile={profile} cloud={cloud}/>;if(screen==='programs')return <Programs go={go}/>;if(screen==='globalPrograms')return <GlobalPrograms go={go}/>;if(screen==='globalDetails')return <GlobalDetails program={selected} go={go}/>;if(screen==='library')return <Library go={go}/>;if(screen==='custom')return <CustomWorkouts go={go} customWorkouts={customWorkouts}/>;if(screen==='builder')return <Builder go={go} saveCustomWorkout={saveCustomWorkout}/>;if(screen==='customDetails')return <CustomDetails program={selected} go={go}/>;if(screen==='exercise')return <ExerciseDetails exercise={selected} go={go}/>;if(screen==='details')return <Details program={selected} go={go}/>;if(screen==='workout')return <Workout program={selected} go={go} saveWorkout={saveWorkout}/>;if(screen==='done')return <Done program={selected} go={go}/>;if(screen==='history')return <HistoryScreen history={history} go={go}/>;if(screen==='evolution')return <Evolution history={history} go={go}/>;return <Profile profile={profile} setProfile={setProfile} go={go}/>;}
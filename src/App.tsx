...
import {Dumbbell,Home,CalendarDays,BarChart3,User,Play,Clock3,ChevronRight,Flame,Pause,RotateCcw,Volume2,Check} from 'lucide-react';
...
export default function App(){
 ...
 useEffect(()=>{if(running&&remaining===0){setRunning(false);setFinished(true); try{navigator.vibrate?.([200,100,200]); const C=window.AudioContext||(window as any).webkitAudioContext; if(C){const c=new C();const o=c.createOscillator();const g=c.createGain();o.frequency.value=880;g.gain.value=.08;o.connect(g);g.connect(c.destination);o.start();o.stop(c.currentTime+.35)}}catch{}}},[remaining,running]);
 ...
 return <div className="app">
  ...
  {tab==='treino'&&<main><section className="workout-head"><span>Treino A</span><h2>{plan.name}</h2><p><Clock3 size={16}/> {plan.duration}</p></section>
   <section className="rest-panel"><div className="rest-title"><div><span>DESCANSO ENTRE SÉRIES</span><h2>{fmt(remaining||rest)}</h2></div><Volume2 size={20}/></div>
    <div className="rest-progress"><div style={{width:`${Math.max(0,Math.min(100,(remaining/(rest||1))*100))}%`}}/></div>
    <div className="rest-controls"><button onClick={running?pauseRest:startRest}>{running?<Pause size={18}/>:<Play size={18} fill="currentColor"/>}{running?'Pausar':remaining>0&&remaining<rest?'Continuar':'Iniciar'}</button><button onClick={resetRest}><RotateCcw size={18}/> Reiniciar</button></div>
    <label>Tempo configurado: <strong>{rest}s</strong><input type="range" min="10" max="300" step="5" value={rest} onChange={e=>saveRest(Number(e.target.value))}/></label>
    {finished&&<div className="rest-done"><Check size={18}/> Descanso encerrado! Próxima série.</div>}
   </section>
   {exercises.map(e=><div className="card exercise-card" key={e.id}><div><h3>{e.name}</h3><p>{e.muscle}</p></div><div className="sets">{e.sets} × {e.reps}</div></div>)}<button className="primary full" onClick={()=>setActive(!active)}>{active?'Treino em andamento':'Iniciar treino'}</button>
  </main>}
